...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($errors)) {
        ...
        if (isset($hashed_password)) {
            $update_sql = "UPDATE users SET full_name = ?, email = ?, phone = ?, address = ?, password = ?, updated_at = NOW() WHERE id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("sssssi", $full_name, $email, $phone, $address, $hashed_password, $user_id);
        } else {
            $update_sql = "UPDATE users SET full_name = ?, email = ?, phone = ?, address = ?, updated_at = NOW() WHERE id = ?";
            $update_stmt = $conn->prepare($update_sql);
