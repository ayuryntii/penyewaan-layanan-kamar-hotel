...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $password = password_hash('guest123', PASSWORD_DEFAULT); // Default password

        // ⚠️ Hanya 5 kolom yang pakai placeholder (?)
        $sql = "INSERT INTO users (full_name, username, email, phone, password, role, status, created_at)
                VALUES (?, ?, ?, ?, ?, 'customer', 'active', NOW())";
        $stmt = $conn->prepare($sql);
        // ✅ 5 variabel → 5 huruf 's'
        $stmt->bind_param("sssss", $full_name, $username, $email, $phone, $password);

        if ($stmt->execute()) {
            $_SESSION['flash_message'] = "Guest added successfully!";
...
<h3 class="card-title"><i class="fas fa-user-plus"></i> Add New Guest</h3>
</div>
<div class="card-body">
<form method="POST">
...
<input type="text" name="username" class="form-control" value="<?= $_POST['username'] ?? '' ?>" required minlength="3">
</div>
</div>
<div class="form-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
<div class="form-group">
